...
`git merge <https://git-scm.com/docs/git-merge>`_ 会产生额外的 commit 记录，如果希望得到更干净、直观的记录，可以使用 `git rebase <https://git-scm.com/docs/git-rebase>`_ 。
rebase 操作还能用于合并多次提交记录。在开发分支 rebase 之后，推到远程需要使用 ``push -f`` ，因为 rebase 会生成新的 commit 顶在 master 最前面。 
不建议在公共分支（如 master）进行 rebase 其他分支的操作，避免出现代码提交记录错乱和浪费存储空间的现象。

.. note::

    协同开发场景下，在开发自己的分支时，要注意合并 master 分支的最新更新： ``git pull origin master`` 或 ``git rebase master`` ，手动解决冲突。
...
8. 【git命令】git rebase合并多次commit操作 
